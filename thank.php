...
if ($_GET['state']) {
	$state = stripslashes(strip_tags(trim($_GET['state'])));
} else {
	$state = false;
}
?>
...
<body class="page__body page--column">
	<div class="header__wrapper header--fix">
		<header class="page__header header page--center" id="header">
			...
			<a class="header__calc button button--primary header__calc--hide" href="/calculator.php">
				<span class="visually-hidden">Расчет стоимости ремонта</span>
			</a>
			<!-- Вызов модального окна с калькулятором -->

			<ul class="schedule list">
				<!-- Список расписания работы офиса -->
	...
	<main class="page__content page--center page--column">
		...
		<section class="examples section section--center">
			<?php if ($state == "true") : ?>
				<h2 class="section__title section__title--inherit">Ваша заявка принята</h2>
				<p class="section__text section__text--inherit">
					Спасибо! С вами скоро свяжутся
				</p>
			<? else : ?>
				<h2 class="section__title section__title--inherit error">Ошибка передачи данных</h2>
				<p class="section__text section__text--inherit">
					Что-то пошло не так и ваш запрос не отправлен.
				</p>
			<? endif; ?>
		</section>
